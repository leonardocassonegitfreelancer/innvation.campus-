...
const translations = {
  en: {
    ...
    eventType: "Type of Event", eventPlaceholder: "Select event type",
    events: [
      { value: "corporate", label: "Corporate Event" },
      { value: "networking", label: "Networking / Drinks" },
      { value: "celebration", label: "Team Celebration" },
      { value: "workshop", label: "Workshop / Retreat" },
      { value: "other", label: "Other" },
    ],
    message: "Message", messagePlaceholder: "Tell us about your event...",
    submit: "Send Message",
    thankYou: "Thank you! We'll get back to you soon.",
  },
  es: {
    sectionLabel: "Contáctanos",
    title: "Reserva la Terraza",
    subtitle: "Cuéntanos sobre tu evento y crearemos una propuesta personalizada.",
    ...
    eventType: "Tipo de Evento", eventPlaceholder: "Selecciona tipo de evento",
    events: [
      { value: "corporate", label: "Evento Corporativo" },
      { value: "networking", label: "Networking / Cóctel" },
      { value: "celebration", label: "Celebración de Equipo" },
      { value: "workshop", label: "Taller / Retiro" },
      { value: "other", label: "Otro" },
    ],
    message: "Mensaje", messagePlaceholder: "Cuéntanos sobre tu evento...",
    submit: "Enviar Mensaje",
    thankYou: "¡Gracias! Te responderemos pronto.",
  },
  it: {
    sectionLabel: "Contattaci",
    title: "Prenota la Terrazza",
    subtitle: "Raccontaci del tuo evento e creeremo una proposta su misura.",
    ...
    eventType: "Tipo di Evento", eventPlaceholder: "Seleziona tipo di evento",
    events: [
      { value: "corporate", label: "Evento Aziendale" },
      { value: "networking", label: "Networking / Aperitivo" },
      { value: "celebration", label: "Celebrazione di Team" },
      { value: "workshop", label: "Workshop / Ritiro" },
      { value: "other", label: "Altro" },
    ],
    message: "Messaggio", messagePlaceholder: "Raccontaci del tuo evento...",
...
export default function TerraceCTA() {
      ...
      <div className="relative max-w-2xl mx-auto px-6">
        <div ref={ref} className={`scroll-animate ${isVisible ? "visible" : ""}`}>
          ...
          <form onSubmit={(e) => { e.preventDefault(); alert(t.thankYou); }} className="space-y-6">
            ...
            <div>
              <Label className={`font-body text-sm ${mutedColor}`}>{t.eventType}</Label>
              <Select value={eventType} onValueChange={setEventType}>
                <SelectTrigger className={`mt-1 bg-white/10 border-white/20 text-white focus:border-primary ${!eventType ? "text-white/30" : ""}`}><SelectValue placeholder={t.eventPlaceholder} /></SelectTrigger>
                <SelectContent className="bg-neutral-dark border-white/20 text-white">{t.events.map((opt) => (<SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>))}</SelectContent>
              </Select>
            </div>
