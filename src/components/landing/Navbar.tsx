      ...
      {mobileOpen && (
        <div className="md:hidden bg-neutral-dark/98 backdrop-blur-lg border-t border-white/10">
          <div className="px-6 py-6 flex flex-col gap-2">
            ...
            {mobileExpanded === "explore" && (<div className="pl-4 flex flex-col gap-1 mb-2">{links.explore.map((link) => (<Link key={link.href} to={link.href} onClick={() => setMobileOpen(false)} className="text-white/60 hover:text-white text-base font-body py-1.5 transition-colors">{link.label}</Link>))}</div>)}
