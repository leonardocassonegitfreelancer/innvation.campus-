...
export default function EventLeadCaptureES() {
  ...
  if (!space) {
    return <Navigate to="/es/organiza-tu-evento" replace />;
  }

  const tr = space.translations.es;
  const amentities = space.amenities.es;
  const otherSpaces = spacesDataset.filter(s => s.slug !== space.slug && s.location === space.location).slice(0, 3);

  return (
    <>
      <SEOHead
        title={`Solicitar - ${tr.name}`}
        description={`Solicita reservar ${tr.name} en Innovation Campus.`}
        path={`/es/organiza-tu-evento/lead?space=${space.slug}`}
      />
      <Navbar />
      <main className="pt-24 bg-muted/30">
        
        <section className="max-w-4xl mx-auto px-6 pt-12 pb-8">
          <Link to="/es/organiza-tu-evento" className="inline-flex items-center gap-2 font-body text-sm text-primary mb-8 hover:underline">
            <ArrowLeft className="w-4 h-4" /> Volver a todos los espacios
          </Link>
        ...
        {otherSpaces.length > 0 && (
          <section className="py-20 bg-background border-t border-border">
            <div className="max-w-4xl mx-auto px-6">
              <h3 className="font-display font-bold text-2xl text-foreground mb-8">¿Dudas? Otros espacios similares</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {otherSpaces.map(altSpace => (
                  <Link key={altSpace.slug} to={`/es/organiza-tu-evento/lead?space=${altSpace.slug}`} className="group block bg-card rounded-2xl overflow-hidden border border-border hover:shadow-lg transition-all">
                     <div className="h-40 overflow-hidden relative">
                       <img src={altSpace.image} alt={altSpace.translations.es.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                     </div>
                     <div className="p-4">
                       <h4 className="font-display font-bold text-lg text-foreground mb-1">{altSpace.translations.es.name}</h4>
                       <p className="font-body text-sm text-muted-foreground">{altSpace.translations.es.capacityText}</p>
                     </div>
                  </Link>
                ))}
              </div>
