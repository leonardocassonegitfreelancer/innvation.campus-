...
export default function BlogES() {
  return (
    <main className="overflow-x-hidden">
      <SEOHead
        title="Blog"
        description="Ideas sobre coworking, trabajo remoto y construcción de tu negocio desde Innovation Campus."
        path="/es/blog"
      />
      <Navbar />
      ...
      <section className="bg-neutral-dark pt-28 pb-16 md:pt-36 md:pb-20">
        <div className="max-w-6xl mx-auto px-6">
          <Link to="/es" className="inline-flex items-center gap-2 text-white/60 hover:text-white text-sm font-body mb-6 transition-colors">
      ...
      <section className="py-16 md:py-24 bg-background">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-8">
            {blogPosts.map((post) => (
              <Link key={post.slug} to={`/blog/${post.slug}`} className="group block border border-border rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 bg-card">
