...
const locations = [
  {
    ...
    link: "/es/malaga-palace",
  },
  {
    id: "malaga-terrace",
    name: "Málaga Terrace",
    subtitle: "Junto al Mar",
...
export default function FindUsES() {
  return (
    <main className="overflow-x-hidden">
      ...
      <section className="pt-32 pb-16 bg-neutral-dark">
        <div className="max-w-6xl mx-auto px-6">
          <Link to="/es" className="inline-flex items-center gap-2 text-white/70 hover:text-white text-sm font-body mb-6 transition-colors">
            <ArrowLeft className="w-4 h-4" />
            Volver al Inicio
          </Link>
          <p className="font-body text-xs uppercase tracking-[0.3em] text-primary mb-2 font-semibold">Nuestras ubicaciones</p>
          <h1 className="font-display italic text-4xl md:text-5xl font-bold text-primary-foreground">Encuéntranos</h1>
          <p className="font-body text-primary-foreground/70 mt-3 max-w-xl">Direcciones, horarios e indicaciones para cada ubicación de Innovation Campus.</p>
          <div className="flex flex-wrap gap-3 mt-6">
            {locations.map((loc) => (
              <a key={loc.id} href={`#${loc.id}`} className="font-body text-sm px-4 py-2 rounded-full border border-white/20 text-white/70 hover:text-white hover:border-primary hover:bg-primary/10 transition-all">
                {loc.name}
              </a>
            ))}
          </div>
      ...
      <section className="py-20 md:py-28 bg-neutral-dark relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-8">
          {locations.map((loc) => (
            <div key={loc.name} id={loc.id} className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm overflow-hidden flex flex-col scroll-mt-24">
              {loc.image != null && (
              ...
              <div className="p-8 flex flex-col gap-4 flex-1">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <a href={loc.mapUrl} target="_blank" rel="noopener noreferrer" className="font-body text-sm text-white/70 hover:text-primary transition-colors underline-offset-2 hover:underline">
                    {loc.address}
                  </a>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <a href={`tel:${loc.phone.replace(/\s/g, "")}`} className="font-body text-sm text-white/70 hover:text-primary transition-colors">{loc.phone}</a>
                </div>
                <div className="flex items-start gap-3">
                  <Mail className="w-4 h-4 text-primary mt-0.5 shrink-0" />
                  <a href={`mailto:${loc.email}`} className="font-body text-sm text-white/70 hover:text-primary transition-colors">{loc.email}</a>
                </div>
                {loc.mapEmbed ? (
                  <div className="mt-auto pt-4 rounded-lg overflow-hidden">
                    <iframe src={loc.mapEmbed} width="100%" height="200" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`Mapa – ${loc.name}`} />
                  </div>
                ) : (
                  <Link to={loc.link} className="mt-auto pt-4">
                    <Button variant="outline" className="w-full font-body text-sm uppercase tracking-widest border-white/20 text-white hover:bg-white/10 hover:text-white">Ver Ubicación</Button>
                  </Link>
                )}
              </div>
            </div>
